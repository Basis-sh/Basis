/**
 * x402 Payment Gate Middleware - Signal Version
 * 
 ...
 * Uses RISK_KV for replay protection (with "tx:" prefix to avoid conflicts).
 */
...
// Prefix for transaction hash keys in RISK_KV (to avoid conflicts with risk data)
const TX_HASH_PREFIX = "tx:";
...
/**
 * x402 Payment Gate Middleware for Signal Worker
 * 
 * Flow:
 * 1. Extract Authorization header (TxHash)
 * 2. Replay Check: Verify hash not in RISK_KV (with "tx:" prefix)
 * 3. Chain Verification: Validate USDC payment on Base
 * 4. Lock: Store hash in RISK_KV with 24h TTL (with "tx:" prefix)
...
export const x402 = async (c: Context, next: Next) => {
  ...
  const kv = c.env.RISK_KV as KVNamespace | undefined;
  ...
  // Use "tx:" prefix to avoid conflicts with risk data in RISK_KV
  const txKey = `${TX_HASH_PREFIX}${txHash}`;
  try {
    const existingHash = await kv.get(txKey);
    ...
    await kv.put(txKey, "pending", { expirationTtl: PENDING_TTL });
  } catch (error) {
  ...
  try {
    ...
    await kv.put(txKey, "used", { expirationTtl: REPLAY_TTL });
  } catch (error) {
